...
hook.Add("EntityTakeDamage", "DoorBreachDamageDetection", function(ent, dmg)
    if not IsValid(ent) or not doorBreachEnabled:GetBool() then return end
    if ent:GetClass() ~= entityType then return end

    if not ent.DoorBreachHealth then
        ent.DoorBreachHealth = doorBreachHealth:GetFloat()
    end

    if ent.DoorBreachExploded then return end
